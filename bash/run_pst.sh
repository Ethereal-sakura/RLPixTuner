...
# Photo Stylization Tuning with cumulative rendering
# Cumulative rendering (isp_inp_original=True) means each step starts from original image

CUDA_VISIBLE_DEVICES=0 python envs/run_sb3_style.py \
  ...
  --loss_type_cont_target first \
  --loss_type_hist yuv \
  --reward_scale 1 \
  --wb_param_dist_mode 1 \
  --env_img_sz 512 \
  --isp style-0 \
